...
parser = argparse.ArgumentParser(description='sub_imagenet features generating')
...
def get_features():
    ...
    def save_features():
        ...
        with torch.no_grad():
            train_features = np.zeros(shape=(12738, n_features), dtype=np.float16)
            train_label = np.zeros(shape=(12738,), dtype=np.int16)
            ...
            test_features = np.zeros(shape=(500, n_features), dtype=np.float16)
            test_label = np.zeros(shape=(500,), dtype=np.int16)
