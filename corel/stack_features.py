...
parser = argparse.ArgumentParser(description='Corel features generating')
...
def get_features():
    ...
    def save_features():
        ...
        with torch.no_grad():
            train_features = np.zeros(shape=(len(train_dataset.imgs), n_features), dtype=np.float16)
            train_label = np.zeros(shape=(len(train_dataset.imgs),), dtype=np.int16)
            ...
            test_features = np.zeros(shape=(len(test_dataset.imgs), n_features), dtype=np.float16)
            test_label = np.zeros(shape=(len(test_dataset.imgs),), dtype=np.int16)
