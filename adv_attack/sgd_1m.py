...
def train(train_label, lr=0.005,monitor=None):
    ...
    with open('220.out', 'a') as f:
        f.write('Training data\'s accuracy: %0.4f\n' %train_acc)
        f.write('Training data\'s loss: %0.4f\n'%train_loss)
...
def test(test_label, monitor=None):
    ...
    with open('220.out', 'a') as f:
        f.write('testing data\'s accuracy: %0.4f\n' %test_acc)
        f.write('testing data\'s loss: %0.4f\n'%test_loss)
    

n_epoch = 210

(_, train_label), (_, test_label) = tf.keras.datasets.cifar10.load_data()
train_label = np.load('label_aug.npy')
...
if resume == False:
    ...
    for i in range(1,n_epoch):
        ...
        print('Cost: %.1f seconds'%(time.time()-a))
    saver.save(sess=sess, save_path='logs/mlp%d/'%au, global_step=batch_size)
elif resume == True:
    saver.restore(sess, 'logs/mlp%d/-%d'%(au,batch_size))
    test(test_label.flatten())
