...
parser = argparse.ArgumentParser(description='STL10 features generating')
...
def get_features():
    ...
    trainset = torchvision.datasets.STL10(root=train_dir, split='train', download=True, transform=test_transform)
    train_loader = torch.utils.data.DataLoader(trainset, batch_size=batch_size, shuffle=False, num_workers=4,
                                               pin_memory=True)
    testset = torchvision.datasets.STL10(root=test_dir, split='test', download=True, transform=test_transform)
    ...
    #     checkpoint = torch.load(save_path + '/ckpt_%d_%d.t7'%(foid, fid))
    ...
    #         os.mkdir(save_path)
    #     torch.save(state, save_path + '/ckpt_%d_%d.t7'%(foid, fid))
    #
    if use_cuda:
    ...
    def save_features():
        net.eval()
        if not os.path.isdir('features0'):
            os.mkdir('features0')
        os.chdir('features0')
        with torch.no_grad():
            train_features = np.zeros(shape=(5000, n_features), dtype=np.float16)
            train_label = np.zeros(shape=(5000,), dtype=np.int16)
            ...
            test_features = np.zeros(shape=(8000, n_features), dtype=np.float16)
            test_label = np.zeros(shape=(8000,), dtype=np.int16)
